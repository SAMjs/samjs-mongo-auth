(function() {
  module.exports = function(samjs) {
    var MongoAuth, debug, getProps, getRef, hasForbiddenKey, hasForbiddenProp, processAuth;
    ...
    getProps = function(user) {
      ...
      id = samjs.auth.getIdentifier(user, this.permissionChecker);
      obj = this._props[id];
      if (obj == null) {
        obj = {
          read: {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          ...
          for (key in ref1) {
            ...
            if (samjs.auth.getAllowance(user, perm, this.permissionChecker) === "") {
    ...
    getRef = function(schema, splitted) {
      var first, ref, ref1, ref2, schemaobj;
      first = splitted.shift();
      schemaobj = schema.path(first);
      if (splitted.length > 0) {
        return getRef(schemaobj.schema, splitted);
      } else {
        return ((ref = schemaobj.options) != null ? ref.ref : void 0) || ((ref1 = schemaobj.caster) != null ? (ref2 = ref1.options) != null ? ref2.ref : void 0 : void 0);
      }
    };
    processAuth = function(obj, mode) {
      ...
      } else if (samjs.auth.getAllowance(user, this.access[mode], this.permissionChecker) === "") {
    ...
    hasForbiddenKey = function(obj, forbidden) {
      ...
      if (forbidden.length > 0 && (obj != null)) {
        util = samjs.util;
        results = [];
        for (key in obj) {
          val = obj[key];
    ...
    samjs.mongo.plugins({
      auth: function(options) {
        ...
        return this.addHook("beforePopulate", function(obj) {
          var allowed, forbidden, i, len, modelname, populate, ref, ref1, ref2;
          ref = obj.populate;
          for (i = 0, len = ref.length; i < len; i++) {
            populate = ref[i];
            modelname = populate.model;
            if (modelname == null) {
              modelname = getRef(this.schema, populate.path.split("."));
            }
            if ((modelname != null) && (((ref1 = samjs.models[modelname]) != null ? ref1.processAuth : void 0) != null)) {
              ref2 = samjs.models[modelname].processAuth(obj, "read"), forbidden = ref2.forbidden, allowed = ref2.allowed;
